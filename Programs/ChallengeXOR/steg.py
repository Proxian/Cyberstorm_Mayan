...
# Bit Method
def bitMethod():
    global EXTRACT, offset, WRAPPER, HIDDEN, interval, SWAP_BIT_DATA
        ...
        while (i < len(WRAPPER)):
            ...
            for k in range(0,8):
                if (i >= len(WRAPPER)):
                    break
                if SWAP_BIT_DATA:
                    tmp |= ((ord(WRAPPER[i]) & 0b00000001) << k) # Add bit to correct location
                else:
    ...
    # Hide file
        ...
        for j in range(0, len(HIDDEN)):
            ...
            for k in range(0, 8):
                ...
                if SWAP_BIT_DATA:
                    WRAPPER[i] = chr(tmp | ((HIDDEN[j + 7 - k] & 10000000) >> 7))
                else:
